...
if (isset($_POST['data'])) {
  ...
  $result = mysqli_query($connection, "SELECT COUNT(*) AS studentcount FROM student WHERE status != 0 AND workshopid IN(SELECT id FROM workshop WHERE approver = '$userid')");
  $data->studentcount = mysqli_fetch_all($result, MYSQLI_ASSOC);

  $result = mysqli_query($connection, "SELECT COUNT(*) AS pendingcount FROM student WHERE status = 1 AND workshopid IN(SELECT id FROM workshop WHERE approver = '$userid')");
  $data->pendingcount = mysqli_fetch_all($result, MYSQLI_ASSOC);

  $result = mysqli_query($connection, "SET NAMES utf8");
  $result = mysqli_query($connection, "SELECT COUNT(*) AS cerificates FROM student WHERE status = 2 AND workshopid IN(SELECT id FROM workshop WHERE approver = '$userid')");
  $data->cerificates = mysqli_fetch_all($result, MYSQLI_ASSOC);

  $result = mysqli_query($connection, "SET NAMES utf8");
  $result = mysqli_query($connection, "SELECT COUNT(*) AS workshopcount FROM workshop WHERE approver = '$userid'");
  $data->workshopcount = mysqli_fetch_all($result, MYSQLI_ASSOC);

  $result = mysqli_query($connection, "SET NAMES utf8");
  $result = mysqli_query($connection, "SELECT COUNT(distinct(email)) AS students FROM student WHERE workshopid IN(SELECT id FROM workshop WHERE approver = '$userid')");
