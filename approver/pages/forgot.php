...
if (isset($_POST['submit'])) {
  ...
  $res = mysqli_query($connection, "SELECT * FROM login WHERE username = '$username' 
                                          AND email = '$email' 
                                          AND status='1' 
                                          AND type = 'approver'");
  if (mysqli_num_rows($res) == 1) {
    ...
    if ($mailstatus == 'Success') {
      $data = mysqli_query($connection, "UPDATE login SET password = '$encpassword' 
                                              WHERE username = '$username' AND `type` = 'approver'");
